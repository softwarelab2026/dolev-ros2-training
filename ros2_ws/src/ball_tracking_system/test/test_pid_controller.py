...
def test_proportional_works():
    pid = PID(4, 0,0)
    assert pid.compute(4, 1) == 16


def test_when_dt_changed_proportional_still_works_the_same():
    pid = PID(4, 0, 0)
    assert pid.compute(4, 1) == 16 and pid.compute(4,100) == 16 


def test_one_integrational_when_4_seconds_passed_compute_works():
    pid = PID(0, 1,0)
    assert pid.compute(4, 4) == 16


def test_two_integrational_when_4_seconds_passed():
    pid = PID(0, 1, 0)
    pid.compute(4, 4) # should return 16
    assert pid.compute(4, 4) == 32

def test_one_derivate_when_second_passed():
    pid = PID(0,0,1)
    assert(pid.compute(4, 1) == 4)


def test_one_derivative_when_4_seconds_passed():
    pid = PID(0, 0, 1)
    assert pid.compute(4, 4)  == 1


def test_two_derivative_when_second_passed():
    pid = PID(0, 0, 1)
